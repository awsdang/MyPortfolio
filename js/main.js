/* THE CITY I BUILD · shared UI. Intro overlay, reveals, signals HUD,
   district label, odometers, NotifyX queue sim + real web push,
...
(function () {
  ...
  var seen = false;
  try { seen = sessionStorage.getItem("city:seen") === "1"; } catch (e) {}
  function endIntro(skip) {
    if (!intro) return;
    intro.classList.add("gone");
    try { sessionStorage.setItem("city:seen", "1"); } catch (e) {}
    if (skip) document.dispatchEvent(new CustomEvent("city:skip"));
  ...
  if (intro) {
    if (seen || reduced) {
      ...
      setTimeout(function () { endIntro(false); }, 1700);
    }
    var skipBtn = document.getElementById("intro-skip");
    if (skipBtn) skipBtn.addEventListener("click", function () { endIntro(true); });
  }
  var replay = document.getElementById("replay");
  if (replay) replay.addEventListener("click", function () {
    try { sessionStorage.removeItem("city:seen"); } catch (e) {}
    window.scrollTo(0, 0);
    location.reload();
  });

  /* ---------------- CV viewer ---------------- */
  document.querySelectorAll("[data-cv-viewer]").forEach(function (link) {
    link.addEventListener("click", function (event) {
      event.preventDefault();
      var viewerUrl = link.href;
      var viewer = window.open(viewerUrl, "_blank");
      if (viewer) {
        try { viewer.opener = null; } catch (e) {}
      } else {
  ...
  /* ---------------- HUD district label + signals ---------------- */
  var hudDistrict = document.getElementById("hud-district");
  var sigCount = document.getElementById("sig-count");
  var lit = {};
  function light(sig) {
    if (!sig || lit[sig]) return;
    var el = document.querySelector('.sg[data-sig="' + sig + '"]');
    if (!el) return;
    lit[sig] = true;
    el.classList.add("on");
    if (sigCount) sigCount.textContent = Object.keys(lit).length + "/5";
  }
  var named = document.querySelectorAll("[data-district][data-name]");
  document.body.setAttribute("data-active-district", "gateway");
  if ("IntersectionObserver" in window && named.length) {
    var dio = new IntersectionObserver(function (es) {
      es.forEach(function (e) {
        if (!e.isIntersecting) return;
        document.body.setAttribute("data-active-district", e.target.getAttribute("data-district"));
        if (hudDistrict) hudDistrict.textContent = e.target.getAttribute("data-name");
        light(e.target.getAttribute("data-signal"));
      });
    }, { threshold: 0.25 });
  ...
  if ("IntersectionObserver" in window && pageFooter) {
    new IntersectionObserver(function (es) {
      es.forEach(function (e) {
        document.body.classList.toggle("footer-visible", e.isIntersecting);
      });
    }, { threshold: 0 }).observe(pageFooter);
  }
  ...
  if (sendBtn) {
    ...
    /* falls back to the sim toast until NOTIFYX_CONFIG has appId / apiKey / vapidPublicKey */
    ...
    var sendRealPush = function () {
      var registered = nx.getState()
        ? Promise.resolve()
        : nx.init({ externalUserId: visitorId() });
      return registered.then(function () {
        return nx.sendTestNotification({
          title: "Hey, you need a system fixed?",
          body: "This push travelled the queue you just watched. · Aws",
          actionUrl: "mailto:awsdang@gmail.com"
        });
      });
    };
    ...
    sendBtn.addEventListener("click", function () {
      ...
      if (nxReady && !nxBusy) {
        nxBusy = true;
        showToast("<b>NOTIFYX</b> · queueing a real push to this browser…");
        sendRealPush().then(function () {
          showToast("<b>NOTIFYX</b> · delivered. Check your notifications ;)");
        }).catch(function (err) {
          showToast("<b>NOTIFYX</b> · " + (err && err.message ? err.message : "push failed · but the queue above never lies."));
        }).finally(function () { nxBusy = false; });
      } else if (!nxReady) {
        showToast("<b>NOTIFYX</b> · campaign queued → workers → delivered.<br>Hey, you need a system fixed? contact me ;)");
      }
    });
  }
